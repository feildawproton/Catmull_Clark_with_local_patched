...
Patch create_patch(unsigned int verts_dim)
{
...
*/
    ...
    for(unsigned int m = 0; m < pat.faces.height; m++)
    {
        for(unsigned int n = 0; n < pat.faces.width; n++)
        {
            ...
            face_value.v1_i = m * pat.verts.width + n;
            face_value.v2_i = face_value.v3_i + 1;
            face_value.v3_i = face_value.v1_i + pat.verts.width;
            face_value.v4_i = face_value.v3_i + 1;
    ...
    for(int i = 0; i < pat.edges_hrzntl.height*pat.edges_hrzntl.width; i++)
    {
        ...
        edge_value.f1_i = i;
        int j = i - pat.edges_hrzntl.width;
    ...
    for(int m = 0; m < pat.edges_vrtcl.height; m++)
    {
        for(int n = 0; n < pat.edges_vrtcl.width; n++)
        {
            ...
            {
                edge_value.f1_i = m*pat.faces.width;
                edge_value.f2_i = m*pat.faces.width + (n - 1);
            }
            else
            {
                edge_value.f1_i = m*pat.faces.width + n;
                if(n == 0)
                    edge_value.f2_i = pat.faces.width*(m + 1) - 1;
                else
                    edge_value.f2_i = m*pat.faces.width + (n-1);
...
int main(int argc, char** argv)
{
    ...
    Patch* pats;
    #pragma omp parallel shared(num_thread_patches, pats)
    {
        ...
        Patch pat = create_patch(patch_dim);
